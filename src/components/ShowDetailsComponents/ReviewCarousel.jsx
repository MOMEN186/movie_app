...
import './Style/ReviweCarousel.css'; // <-- New, shared CSS
...
const ReviewCarousel = ({ reviews, title }) => {
  ...
  // Custom arrow components (show on hover)
  ...
  return (
    ...
    >
      ...
      <CustomPrevArrow
        onClick={() =>
          handleSelect(index - 1 < 0 ? reviewGroups.length - 1 : index - 1)
        }
      />
      ...
      <CustomNextArrow
        onClick={() => handleSelect((index + 1) % reviewGroups.length)}
      />
    </div>
  );
};
